...
## Block Diagram
        ...
        ▼                      ▼                      ▼
┌────────────────┐   ┌────────────────┐   ┌────────────────┐
│   DHT22        │   │ Status LED     │   │ UART Header    │
│ GPIO26         │   │ GPIO2          │   │ TX=GPIO1       │
│ Temp/Humidity  │   │ System Status  │   │ RX=GPIO3       │
└───────┬────────┘   └────────────────┘   └────────────────┘
        │
        ▼
┌───────────────────────────────────────────────────┐
│                 ESP32-WROOM-32                    │
│                                                   │
│ GPIO23 ─────────► Relay Driver 1                  │
│ GPIO22 ─────────► Relay Driver 2                  │
│ GPIO21 ─────────► Relay Driver 3                  │
│ GPIO19 ─────────► Relay Driver 4                  │
│                                                   │
│ GPIO18 ─────────► Relay LED1                      │
│ GPIO5  ─────────► Relay LED2                      │
│ GPIO17 ─────────► Relay LED3                      │
...
└───────────────┬───────────────────────────────────┘
 ...
 SW1(GPIO34) ─┐
 SW2(GPIO35) ─┼─► Debounce ► Event Queue ► Relay Control
 SW3(GPIO32) ─┤
 SW4(GPIO33) ─┘
 ...
 GPIO23 ─► Driver ─► Relay1 ─► Load1
 GPIO22 ─► Driver ─► Relay2 ─► Load2
 GPIO21 ─► Driver ─► Relay3 ─► Load3
 GPIO19 ─► Driver ─► Relay4 ─► Load4
